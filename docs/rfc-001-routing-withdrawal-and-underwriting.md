...
- **Status:** Draft for discussion, revision 6. Not accepted, audited, or authorized for deployment; an executable reference model of the revision-5 rules with the revision-6 corrections is merged in `sim/intendment/rfc001.py`.
- **Date:** 2026-09-06, revision 6, which folds what the implementation review of the reference model (PR #10) found. Revisions 5 (4271161), 4 (73224e4), 3 (bb63e08), 2 (d798fee) and 1 (ba25e22) are in the repository history.
...
| Revision 4 carried the maintainer's review issues #5 to #9. | Revision 5 states final-offer convergence as a hypothesis with the metrics that test it; freezes the severity case record when adjudication opens; fully collateralizes first-instance cost shifting and caps reimbursable litigation liability ex ante; specifies the reservation-ticket and concession-mandate lifecycles; and makes the deferral of reserve underwriting consistent, superseding the stronger wording of the revision 2 and 3 rows above. | Convergence is a tendency, not a theorem; a case must be immutable once jurors work on it; a reimbursement is only as good as its collateral; races need rules; a counterexample is not a category proof. |
| Revision 5 said an unreserved submitter posts the severity fee at submission, and let a missing cost bond block a severity case. | Revision 6 gives a request without a qualifying ticket the claim as its default, with the requester as the only party who may open a severity case, at its own cost and without reimbursement; lets a claimant open above the available backing with reimbursement capped at that backing; and requires an explicit rung at challenge. | A stock host cannot take a fee at submission, so the rule had to move to the default side; a cap bounds a reimbursement, not the right to adjudicate; an unstated claim must not mean the maximum. Found by the reference model in PR #10. |
...
- **The case is frozen when adjudication opens.** The transition that opens a severity case snapshots an immutable record: both final rungs and their amounts, the policy and rubric version, the original case id and evidence reference, the severity court configuration, the cost-shifting rule and the cost-shift bond, and the funding and appeal deadlines. After it neither side can move, later offers or messages do not touch the live case, the ballot names `REQUESTER_FINAL_OFFER` and `CHALLENGER_FINAL_OFFER` from the record, and every appeal inherits the same two positions and version. Equal positions before opening settle directly and no case is created. Every severity ruling therefore maps to one exact monetary outcome known when the case is created.
...
1. **Positions.** The challenger's rung, stated explicitly at challenge and never defaulted to the maximum, is a claim; the requester's rung, stated at concession or after the merits ruling, is an admission. During a short window each side may move only toward the other, the baseline's monotone discipline for asks. Equal positions settle without court. A requester who states nothing stands at the lowest rung.
2. **Burden and silence.** For a request that had a qualifying ticket at submission, the claimant fronts the first-instance fee and opens the severity case to get more than the admission. The requester's silence is not a loss: the jurors still choose between the two positions, which keeps S1. If nobody opens a case within the window, the admission applies. A tie or a refusal to arbitrate means the claim was not established, and the admission applies. **For a request without a qualifying ticket at submission the burden is reversed**: the claim is the default, the requester is the only party who may open a severity case, it fronts the fee itself and is never reimbursed, and a tie or refusal leaves the claim standing. Qualification is read from the actual submission, so a timely ticket bound late keeps the normal mode, and a stranger's, cancelled, same-block, expired or wrong-policy ticket does not qualify. Whether a request was backed is a matter of record at submission, never of later top-ups.
3. **Cost shifting, fully collateralized.** Award risk and litigation-cost risk are kept apart. The first-instance severity fee `q_s` is part of every reservation, held separately from the award backing, and the opening transition locks it as a **cost-shift bond**. If the jurors pick the claim, the bond reimburses the fee the claimant fronted; if they pick the admission, or tie, or refuse, the bond returns to the requester and the claimant bears its fee. This is the offer-of-judgment rule with nothing left to interpret, and every reimbursement is backed by cash: reimbursable litigation liability is capped ex ante. If the resolver's fee at opening exceeds the available backing, the claimant may still open; the reimbursement is frozen at the smaller of the fee and the backing, the claimant bears the rest even on winning, and the requester may top up before opening but cannot block adjudication by withholding a top-up. A later policy may add a small disclosed allowance on top (open decision 13), never an uncapped one. A request without a reservation has no bond and falls under the reversed burden of rule 2.
4. **Rounds.** Appeals are voluntary further litigation under the court's own stake rules, funded by whoever appeals and by anyone who crowdfunds a side; they are never reimbursable under the cost-shifting promise, so nobody can manufacture debt by appealing. The case still ends early on its own: the most a side can gain is the distance between the positions, while appeal stakes triple per round.

**Why the positions are expected to move, and why that is a hypothesis.** Under split-the-difference arbitration each side gains by exaggerating; under final-offer arbitration the side closer to the jurors' view wins everything, so each side's best position moves toward what it expects the jurors to find, and if the truth lies between two positions whoever moves there first wins. That is the intended pressure, and it is documented for human arbitration under shared expectations. It is not a guarantee: parties who disagree about the jurors' view, value the upside differently, or find speculation cheap still reach court, and a cheap first instance makes speculation cheaper, which is what the cost-shift bond is for. Final-offer severity is intended to push positions toward the parties' expectations of the adjudicated result; whether a finer ladder increases settlement, narrows the distance between positions, or only improves compensation precision is empirical, and experiment 4 measures it. A court forced onto a wrong rung because neither side moved remains the price of refusing to move.
...
**Races and liveness.** Execution and escalation compete for one terminal transition: exactly one wins, and a losing or reverted execution consumes neither a case nor budget. Executing against a case already settled by other means is a no-op that consumes nothing. Consumption is atomic and idempotent under concurrent keepers, so one execution is credited. When the remaining balance or case cap is insufficient the case stays individually settleable and escalatable; a mandate is never a liveness dependency. Nothing executes after expiry, and unspent prefunding becomes claimable under a stated rule. A Merkle scope covers only the enumerated existing cases, never future challenges to related submissions. Permissionless execution is an availability property, not a promise that someone calls: where completion before a deadline matters, the interested caller is the requester who funded the mandate, and that assumption is named rather than assumed (open decision 14).
...
reservedBond    = sum over exposed requests of maximumExposure
maximumExposure = the promise gap + q_s, the first-instance severity fee (section 4.3)
```

- **Reserve before submitting.** On an unmodified host the reservation is a ticket that binds the host, the requester, the item id, the expected request index, the maximum exposure, a nonce and an expiry. The item id is the hash of the descriptor and the request index is the host's current request count for that item, both known in advance. The requester binding is essential, not decorative: anyone can submit the same descriptor bytes, and without it a stranger's submission would consume someone else's collateral. Only a matching request by the bound requester consumes the ticket. The policy requires a valid ticket at submission, and the evidence display shows "reserved, or standing at submission: yes / no" from the wrapper's events and the host's request data, so jurors decide it at no cost. The ticket must exist in a block strictly before the submission.
- **Unused tickets.** If the submission never happens, or another requester consumed the bound index first, the owner cancels after proving that no matching request consumed the ticket; cancellation releases the lock, and a cancelled or expired ticket can never be consumed later.
- **Release only when every financial liability is terminal.** Unchallenged and executed, or requester wins: full release. A debt below the reserved maximum: pay it and release the rest. The full amount owed: pay it to the creditor. Terminal means after any challenge, appeal, settlement, the conditional severity case, and host-specific latent exposure; host membership being decided is not enough. Release is exactly once and permissionless once the terminal predicate is provable from host and wrapper state.
...
Unchanged in substance from revision 1 and further from the critical path. Liquidity, loss-absorbing capital, and committed exposure are three different quantities; fully funded lending to an unmodified host needs cash for every advanced deposit; a contingent guarantee needs an adapted host and an explicit failure outcome. Section 5.8 adds the constraint for the one construction it rejects, an unsecured, no-recourse payout to a claimant the borrower may control: such an advance is bounded by the fee burned in a court loss, which a cheap first instance makes negligible. Fully funded lending, contingent guarantees, first-loss or collateralized credit, and fractional reserves are distinct constructions; they are deferred here, not judged. Public capital solicitation, lending, and guarantees require a jurisdiction-specific legal review before any launch; this RFC supplies no legal conclusion.
...
| Severity ladder and final-offer severity cases | the wrapper's second dispute type, in which the wrapper is the arbitrable in its own right: dispute creation, evidence, meta-evidence, appeal crowdfunding, ruling execution; and the policy ladder | wrapper and policy | by a timelocked policy change where the policy is mutable, otherwise with the next registry; audit surface beyond stage 1a, shipped with the credit module |
...
| Reserve underwriting | a separate threat model, self-dealing analysis, reserve sizing, a correlated-default model, pricing, recourse and recovery, adversarial simulations | research track | deferred; not on the implementation critical path |
...
| Severity freeze | A position update racing the opening transition, duplicate funding, a stale signed offer, and an appeal after an attempted position change never change the frozen record; every ruling and appeal uses the two frozen positions. |
| Cost-shift bond | Claimant wins, requester wins, tie and refusal, a fee change between reservation and opening, a fee above the backing, inability to fund, and appeals: the bond pays or returns exactly once, reimbursement never exceeds the smaller of the fee and the backing, appeals are never reimbursed, and no reimbursable liability exceeds the cap. |
| Unbacked request | A request without a qualifying ticket resolves to the claim by default, only the requester may open a severity case, it is never reimbursed, a late-bound timely ticket restores the normal mode, and a foreign, cancelled, same-block, expired or wrong-policy ticket does not. |
...
| Mandates | Execution versus escalation with exactly one winner, execution against a settled case, concurrent keepers, an exhausted cap or balance, expiry with refund, replay, and scope expansion; budget is consumed only by a successful case transition, and no case needs another case processed to terminate. |
...
| Reservations | A ticket is bound to one requester and one request identity, cannot be withdrawn or reused while that request can produce liability, releases exactly once when every liability including the severity case is terminal, pays partial and full debts correctly, and survives retirement and concurrent claims. An unused ticket is cancellable only after proven non-consumption; a cancelled or expired ticket is never consumed; another requester's submission at the bound index cannot consume it. |
...
**Result, 2026-09-06, superseded the same day.** 50 tests passed on branch `exp/credited-model`, the 27 stage-1a tests untouched and 23 new, with money conservation asserted at every step; the band is in section 5.4. The model tightened six rules, now folded into sections 4.2, 4.3, 5.2, 5.3 and 8: the untiered funding flip, cost shifting bounded by wrapper-held money, escrow instead of same-transaction cash, the host's open-request count in the bonding check, a bond withdrawal rule, and standing across a switch. It also confirmed that the griefing cost with cheap challenges is exactly the fee plus the challenger deposit and nothing else, which is open decision 2. The batch move it modelled is superseded by the mandates of section 4.4, and its flat tiers by the two-question model of section 4.2.

**Reference model, merged 2026-09-06 (PR #10, `sim/intendment/rfc001.py`).** An integer-accounting implementation of the revision-5 rules with reservations, separate cost bonds, frozen final offers, final-outcome reimbursement, bounded appeal crowdfunding, unsecured debt kept apart from cash, prefunded Merkle-scope mandates, atomic rollback and global invariants; 111 tests, 138 with the baseline. Its review produced the three corrections of revision 6. `docs/rfc-001-implementation.md` records its decisions and boundaries; it is a model, not a contract.

### Experiment 4: ladder convergence

In the executable model, run the severity negotiation for ladders of three, five and eight rungs under several distributions of the jurors' view and of the parties' beliefs about it, with both sides moving only toward each other, the cost-shift bond locked at opening, and the claimant fronting the cheap first instance. Measure at least: the initial and final distance between positions, the settlement rate before court, the severity-court rate, total adjudication fees, time to resolution, compensation error against the simulated juror view, and how often the winning final offer is far from that view. **Advance when** the measurements show what each ladder length buys: a longer ladder is worth having if it improves compensation precision even where it does not raise the settlement rate, and the final-offer architecture is kept unless the experiment falsifies its incentive hypothesis.
...
13. The optional litigation allowance on top of the cost-shift bond, and whether a reputable submitter may carry it as capped unsecured debt.
14. The keeper assumption for mandate execution and reservation release near deadlines: who is paid to call.
...
One candidate constraint for the spine, kept narrow so that it does not prejudge future underwriting: **no system-funded guarantee may create positive expected value for a promisor who can deliberately trigger the covered loss and direct the payout to an account it controls, absent sufficient first-loss, recourse, collateral, or another independently costly constraint.** It generalizes S4 to guarantees; the reserve-free form of section 5 satisfies it trivially, and any future reserve construction has to show how it does.
...
- Issues [#5](https://github.com/lovon-spec/intendment/issues/5), [#6](https://github.com/lovon-spec/intendment/issues/6), [#7](https://github.com/lovon-spec/intendment/issues/7), [#8](https://github.com/lovon-spec/intendment/issues/8) and [#9](https://github.com/lovon-spec/intendment/issues/9): the maintainer's review of revision 4, folded in revision 5.
- [PR #10](https://github.com/lovon-spec/intendment/pull/10) and [`docs/rfc-001-implementation.md`](rfc-001-implementation.md): the reference model and its review, folded in revision 6.
