...
- **Status:** Draft for discussion, revision 5. Not accepted, implemented, audited, or authorized for deployment.
- **Date:** 2026-09-06, revision 5, which folds the maintainer's review issues #5 to #9. Revisions 4 (73224e4), 3 (bb63e08), 2 (d798fee) and 1 (ba25e22) are in the repository history.
...
2. **Burden and silence.** The claimant fronts the first-instance fee and opens the severity case to get more than the admission. The requester's silence is not a loss: the jurors still choose between the two positions, which keeps S1. If nobody opens a case within the window, the admission applies. A tie or a refusal to arbitrate means the claim was not established, and the admission applies.
3. **Cost shifting, fully collateralized.** Award risk and litigation-cost risk are kept apart. The first-instance severity fee `q_s` is part of every submitter's reservation or prepayment, and the opening transition locks it as a **cost-shift bond**. If the jurors pick the claim, the bond reimburses the fee the claimant fronted; if they pick the admission, or tie, or refuse, the bond returns to the requester and the claimant bears its fee. This is the offer-of-judgment rule with nothing left to interpret, and every reimbursement is backed by cash: reimbursable litigation liability is capped ex ante at `q_s`. A later policy may add a small disclosed allowance on top (open decision 13), never an uncapped one, and a submitter without a reservation posts `q_s` at submission, which at a cheap first instance is a fraction of an xDAI.
...
| Cost-shift bond | Claimant wins, requester wins, tie and refusal, a fee change between reservation and opening, inability to fund, and appeals: the bond pays or returns exactly once, appeals are never reimbursed, and no reimbursable liability exceeds the cap. |
