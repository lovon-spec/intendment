...
**Normal flow, unchanged until a dispute.** The adapter requests a price from the front exactly as it does from the OO. Proposers propose on the front and post the bond; disputers dispute on the front within the liveness and post the bond. What differs is when the adapter hears about a dispute: the OO calls `priceDisputed` in the dispute transaction, and the adapter resets the question on that first call. The front withholds the callback for the length of the window. A conceded dispute therefore never reaches the adapter, and its one reset is not consumed; a dispute that is forwarded at the deadline is reported then, and the adapter resets exactly as it does today. One callback timeline, stated: dispute on the front, window, then either concession with no callback or forwarding with the callback.

**The window.** A dispute on the front opens the concession window instead of escalating. Only one move exists in it, by design (section 5 of the design, stage 4, and requirement R15): the proposer may **concede**, retracting the proposal and paying the disputer what a lost vote would have paid them. Everything UMA's vote would have taken out of the parties is preserved: the disputer receives the winner's share of the proposer's bond, and the oracle's share is paid to UMA's Store just as a vote would have paid it. The economics of a concession are identical to losing the vote; only the days of voting are saved, and for a market that is the prize. There is no disputer withdrawal and no price to negotiate, because the people whose money rides on the outcome are not at the table.

**After a concession.** The request stays open on the front and a new proposal may be made at once, with a fresh liveness. Because the adapter was never told, it is not reset and its one reset is not consumed, so a market can survive more than one wrong proposal before the vote becomes unavoidable. A disputer who knows the right answer is usually the next proposer. The proposer's bond is settled as a lost vote would settle it: the disputer's share to the disputer, the oracle's share to UMA's Store; the disputer's bond returns.
